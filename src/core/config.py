...
class Settings(BaseSettings):
    ...
    model_path: Path = Field(default=MODEL_DIR / "cnn_lstm_universal.pt", alias="MODEL_PATH")
    onnx_path: Path = Field(default=ONNX_DIR / "cnn_lstm_universal.onnx", alias="ONNX_PATH")
...
class SensorReading(BaseModel):
    ...
    voltage: float = Field(..., ge=2.0, le=5.0)
    current: float = Field(..., ge=-50.0, le=50.0)
...
class InferenceResult(BaseModel):
    ...
    battery_id: str
    rul_percent: float = Field(..., ge=0, le=100, description="Predicted RUL in percentage")
