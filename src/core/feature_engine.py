...
class FeatureEngine:
    ...
    def _extract_health_indicators(self, cycle_df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Ensure fallback for v_mean, i_mean, t_mean if not generated
        if "v_mean" not in hi_df.columns: hi_df["v_mean"] = 3.7
        if "i_mean" not in hi_df.columns: hi_df["i_mean"] = -1.0
        if "t_mean" not in hi_df.columns: hi_df["t_mean"] = 25.0
...
if __name__ == "__main__":
    ...
    rprint(table)
    rprint(f"\n[bold green]Success Feature matrix saved columns:[/bold green] {list(feature_df.columns)}")

    # Save to disk
    out_path = DATA_DIR / "feature_matrix.parquet"
    feature_df.to_parquet(out_path, index=False)
    rprint(f"[bold cyan]Success Saved to:[/bold cyan] {out_path}")
